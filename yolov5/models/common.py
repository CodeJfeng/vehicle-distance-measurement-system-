...
#         self.m = nn.Sequential(*[Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)])
#         self.channel_attention2 = ChannelAttention2(c2, 16)
#         self.spatial_attention2 = SpatialAttention2(7)
#
#         # self.m = nn.Sequential(*[CrossConv(c_, c_, 3, 1, g, 1.0, shortcut) for _ in range(n)])
#
#     def forward(self, x):
#         # 将最后的标准卷积模块改为了注意力机制提取特征
#         return self.spatial_attention2(
#             self.channel_attention2(self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), dim=1))))
